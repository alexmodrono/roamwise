...
const archive = resolve('packages/cli/alexmodrono-roamwise-0.1.0.tgz');
...
const executable = join(temp, 'node_modules/@alexmodrono/roamwise/dist/cli.mjs');
try {
  ...
  const { stdout } = await exec('npm', ['exec', '--yes', `--cache=${join(temp, 'npm-cache')}`, `--package=${archive}`, '--', 'roamwise', 'open', join(temp, 'node_modules/@alexmodrono/roamwise/dist/viewer/trips/minimal-trip.yaml'), '--print-url', '--json'], { cwd: runDirectory, env });
  ...
  const { stdout: validation } = await exec(process.execPath, [executable, 'validate', join(temp, 'node_modules/@alexmodrono/roamwise/dist/viewer/trips/minimal-trip.yaml'), '--json'], { cwd: temp, env });
